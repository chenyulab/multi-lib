...
%
%   args.threshold (int)
%       -- threhold define the overlap proportion between cevent data and
%          speech utterance, keep the utterance that is 
%          (cevent_length)/(utterance_length) > threhold
%
%   args.whence
...
function [extracted_data_set] = extract_speech_in_situ(subexpID,cevent_var,category_list,output_filename,args)
    ...
    for w = 1:num_wordlist
            ...
            for j = 1:numel(speech_var)
                ...
                if is_default_timewindow == 1
                    ...
                    if ~isempty(target_words)
                        ...
                        count_vec = count_target_words(target_words,speech_var(j).words);
                ...
                elseif is_default_timewindow == 2
                    ...
                    for k = 1:size(onset,1)
                        ...
                        if ~isempty(target_words)
                            % calculate basic token measures per matching instance
                            count_vec = count_target_words(target_words,sub_utt);
        ...
        extracted_data_set{end+1} = extracted_data;
