...
function count_cat_word_pair_freq(input_csv,sub_col,cat_col,utt_col,output_excel)
    ...
    for i = 1:size(data,1)
        ...
        count_list(cat_idx) = count_list(cat_idx) + 1;
    end
    ...
    % Filter out stopwords if flag is 0
    if ~keep_stopwords
        final_word_order = setdiff(final_word_order, stopWords_list, 'stable');
    end
    ...
    headers = [{'cat_label'}, {'total_instance'},final_word_order];
    overall_table = cell2table(horzcat(labels, num2cell(count_list), num2cell(sorted_matrix)), "VariableNames", headers);
    ...
    for s = 1:length(sub_list)
        ...
        sub_table = cell2table(horzcat(labels,num2cell(count_list),num2cell(sorted_matrix)),"VariableNames",headers);    
