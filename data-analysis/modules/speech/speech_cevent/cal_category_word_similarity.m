...
%       - args.extaStopWords (string cell array)
%           - list of extra stops words that will be excluded during calculation,
...
% 
%%%
function expanded_data = cal_category_word_similarity(input_filename,output_filename,args)
    ...
    stopwords_list = [args.extaStopWords, stopWords];
    excludeSubjects = args.excludeSubjects;

    table_data = readtable(input_filename,'VariableNamingRule', 'preserve');
    headers = table_data.Properties.VariableNames;
    % insert cat word and similarity col followed catValue and speechWord
    headers(speechWord_col) = [];
    headers = [headers,'instance_id','cat_label','speech_word','similarity'];
    ...
    for i = 1:size(table_data,1)
        ...
        % Remove punctuation and extrac space
        clean_text = regexprep(text, '[^\w\s]', ''); 
        clean_text = regexprep(clean_text, '\s+', ' ');       
        % Split into words
        ...
        for j = 1:length(words)
            ...
            row = [row_data,{instance_id},{cat_label},{word},{similarity}];
