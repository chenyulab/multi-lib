...
for i = 1 : size(data,1)
    ...
    % calculate (O_t - O_d)/max(O_t, O_d) in 20 column 
    for i = 1 : size(results,1)
        if results(i,14) > results(i,19)
            results(i,20) = (results(i,14)-results(i,19))/results(i,14);
        else
            results(i,20) = (results(i,14)-results(i,19))/results(i,19);
        end
...
t.Properties.VariableNames  ={'subject ID','Exp ID','onset','offset','target ROI','trial id','instance ID',...
