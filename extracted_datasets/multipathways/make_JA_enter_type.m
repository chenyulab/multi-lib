...
% variables for each subject in the input experiments.
%
% Sample function call: main_multipathways([12 15 27 49 58 59 71 72 73 74 75 91 351 353], [1 2], 0)
%%%
    ...
    for idx = 1:length(exp_ids)
        ...
        for j = 1:length(JA_types)
            ...
            for i = 1 : length(event)
                ...
                index_inhand = intersect(find(data(index_rest,inhand_col)>inhand_prop),find(data(index_rest,inhand_col)>data(index_rest,inhand_other_col)));
                JA_follow_type{i}(index_rest(index_inhand)) = 3;
    
                % hand following self
                index_rest = find(JA_follow_type{i}==1);
                index_inhand_self = intersect(find(data(index_rest,inhand_self_col)>inhand_prop),find(data(index_rest,inhand_self_col)>data(index_rest,inhand_self_other_col)));
                JA_follow_type{i}(index_rest(index_inhand_self)) = 4;
                ...
                index_face_hand_other = find(data(index_rest,inhand_other_col) > 0);
                ...
                index_face_no_hand = find(data(index_rest,inhand_col)==0 & data(index_rest,inhand_other_col)==0);
